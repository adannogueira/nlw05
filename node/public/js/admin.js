...
function call(id) {
  const connection = allConnections.find(connection => connection.socket_id === id)
  inSupport.push(connection)
...
socket.on('admin_receive_message', params => {
  const connection = inSupport.find(connection => connection.socket_id === params.socket_id)
  ...
  createDiv.innerHTML += `<span class="admin_date">${dayjs(params.message.created_at).format(
    'DD/MM/YYYY HH:mm:ss'
  )}</span>`
